...
- **Krok 162/163 (2.0.70, hotovo):** doplněna `-keepclassmembers` pravidla pro Gson modely (`Question`, `LessonProgress.LessonStateJson/PracticeStore`), zapnuto `isShrinkResources = true`. Ověřeno plným `assembleRelease` (funguje i bez `keystore.properties` díky fallbacku na debug signing) — release APK 7,53 MB → 6,83 MB (~9,3 %). Viz CHANGELOG 2.0.70.
