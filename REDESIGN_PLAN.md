...
| 159 | Accessibility audit | 13 | ✅ (WCAG AA kontrast spočítán a opraven ve všech 3 vizuálních stylech; touch target doplněn; ~30 míst s natvrdo `AccentCyan`/`WarningAmber` jako textovou barvou dořešeno v 2.1.4 — viz poznámka níže) |
...
- **Fáze 13, krok 155 + 165** — zbývá: odstranění starých stylů/témat, finální QA + release checklist.
- **Krok 156/159/160 (2.0.70, částečně hotovo):** recomposition audit + reduced motion/low power podpora (`ui/util/ReducedMotion.kt`, `ui/util/PerformanceMode.kt`) + contentDescription audit — viz CHANGELOG 2.0.70. **Zbývá:** systematické měření recomposition counts přes Layout Inspector (krok 156 bod 1), kontrola kontrastu WCAG AA nástrojem (krok 159 bod 4).
- **Krok 157/158** — zbývá beze změny: FPS profiling animací a Coil `ImageLoader` konfigurace (shimmer placeholder/retry) nebyly v této dávce řešeny.
...
## Session 2026-07-25 (2.1.4) — dořešení otevřeného zjištění z kroku 159

- **Krok 159 dořešení (fixní `AccentCyan`/`WarningAmber` jako textová barva):** systematicky projito všech ~45 výskytů `AccentCyan`/`WarningAmber` v `app/src/main/java/cz/autokolk/ui/`. Rozlišeno mezi (a) textovým/informačním použitím (`Text`/`Icon` s popiskem) na THEME-AWARE pozadí — reálný AA problém (`WarningAmber` na bílém ~1.41:1, `AccentCyan` ~1.54:1, oba hluboko pod 4.5:1) a (b) čistě dekorativním použitím (gradient/border/glow/pozadí/výplň progress baru) nebo místy s **pevně tmavým pozadím bez ohledu na zvolený režim** (scrim overlaye `LevelUpOverlay`/`AchievementUnlockOverlay`/`StreakMilestoneOverlay`/`RandomEventOverlay`, `BonusWheelDialog`/`MysteryBoxDialog` s `.background(DarkSurface)`, `CoinsSheet`/`StreakSheet` s `containerColor = DarkSurfaceVariant`, `SplashScreen` s `DarkBackground`, `ShareCardGenerator` kreslící fixní tmavou PNG kartu) — tam byl původní kontrast už v pořádku (>10:1) a nic se neměnilo.
  - **Přístup:** místo přepisu na `MaterialTheme.colorScheme.primary`/`error` (což by u Classic light stylu stále nesplnilo AA — `LightAccentCyan` samo o sobě má na bílém jen ~2.4:1, proto bylo u `onPrimary` v 2.1.2 nutné přejít na tmavý text) byly do `ui/theme/Color.kt` doplněny dvě nové zatemněné, stejně sladěné varianty: `AccentCyanText` (`#00728A`, 5.6:1 na bílém/5.1:1 na `LightBackground`) a `WarningAmberText` (`#7F6B00`, 5.2:1 na bílém/4.8:1 na `LightBackground`) a k nim composable helpery `accentCyanText()`/`warningAmberText()`, které přes `LocalIsDarkTheme` vrací zatemněnou variantu ve světlém režimu a původní jasnou barvu v tmavém (tam je kontrast > 10:1, není důvod ji měnit). Odstín zůstává stejný, mění se jen lightness — stejný princip, jaký byl použit u `ErrorRed`/`onPrimary`/`onTertiary` v kroku 159 (2.1.2).
  - **Opraveno (~20 reálných textových/ikonových míst):** `ChangelogScreen` (verze karty), `SettingsScreen`/`SettingsWidgets` (verze appky, výchozí `valueColor` u `ClickableSetting`), `StreakScreen` a `TopBar` (velké číslo streaku/mincí + ikony v horní liště), `BottomNavBar` (vybraná ikona+popisek ve spodním menu), `QuizScreen`/`QuizTopBar`/`TestQuizSession` (tlačítko "Pokračovat" v dialogu, combo streak zpráva, countdown časovače pod 5 minut), `ResultsComposeScreen` (štítek "Body", tlačítko "Zkusit znovu"), `AlexScreen` (ikona chipu "Obchod", text "Hlad zmrazen"), `OnboardingScreen` (fajfka u vybraného denního cíle, zpětná vazba u ukázkové otázky), `AchievementsScreen` (ikona hvězdy u odemčeného úspěchu), `WeeklyXpScreen` (součet XP, osobní rekord), `PracticeScreen` (návrhy vyhledávání, štítek podkategorie + expand ikonka, odkaz na nejhorší otázku, ikonka dokončené kategorie).
  - **Otevřená otázka pro uživatele (neřešeno, ponecháno konzervativně):** barva palce/dráhy přepínače (`Switch`) v `SettingsWidgets.SwitchSetting` (`checkedThumbColor`/`checkedTrackColor = AccentCyan`) — jde o ovládací prvek (ne text/ikona s popiskem), takže striktně mimo zadaný rozsah "Text/Icon v kontextu s textem"; ponecháno k samostatnému posouzení, zda i tam má smysl AA-safe variantu nasadit.
  - Ověřeno `./gradlew.bat assembleDebug --no-daemon` — build bez chyb. Verze bumpnuta na 2.1.4 (`versionCode` 66).
