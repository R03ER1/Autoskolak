...
## [2.2.13] - 2026-08-05
- Redesign: Mystery box vyměnil koncept "dárečku" za "truhlu" (treasure chest) — nová ikona zavřené truhly (`ic_chest_closed`, zdroj Tabler Icons, MIT licence, viz `mediaassets/src/main/assets/icons/treasure-chest.svg`) a vlastní ručně dokreslená otevřená varianta (`ic_chest_open` — žádná CC0/MIT sada nemá hotový "open chest" pandán ve stejném line-art stylu, jediný nalezený je z game-icons.net pod CC BY 3.0/jiný vyplněný styl, proto je nakreslená jako derivát se stejnou 24×24 mřížkou a tloušťkou čáry). `MysteryBoxGraphic` teď při otevření přepíná CELOU ikonu ze zavřené na otevřenou truhlu (dřív jen malý odznak s fajfkou navíc) a pozadí sjednoceno se stejným tmavým radiálním gradientem, jaký má `WheelGraphic` za bonusovým kolem, místo dřívějšího žluto-tyrkysového gradientového čtverce.
- Feature: intenzivnější animace otevření Mystery boxu — truhla se před otevřením krátce "zatřese" (shake), po odhalení odměny "vyskočí" pružinovým (spring) odskokem s přestřelením a krátce zazáří zlatou září. Respektuje systémové "odstranění animací"/power-save (`rememberLowPerformanceModeEnabled`) — v tom režimu se zatřesení a spring odskok vynechají, zůstává jen krátké zjednodušené zvětšení. Vibrace při otevření zůstává stejná jako dřív (`HapticFeedback.onAchievement`), jen teď spouští současně s vizuálním "pop" efektem.
