...
class TestAttemptActivity : AutokolkActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Replace central content with the test attempt layout while keeping header and bottom nav
        val titleView = findViewById<TextView>(R.id.pageTitle)
        val contentContainer = titleView.parent as? ViewGroup
        contentContainer?.removeAllViews()
        layoutInflater.inflate(R.layout.activity_test_attempt, contentContainer, true)

        // Wire the start-test button to launch MainActivity in test mode
        findViewById<com.google.android.material.button.MaterialButton>(R.id.startTestButton)?.setOnClickListener {
            val intent = android.content.Intent(this, MainActivity::class.java)
            intent.putExtra(MainActivity.EXTRA_IS_TEST_MODE, true)
            startActivity(intent)
        }

        // Wire the statistics link to open a blank stats page
        findViewById<TextView>(R.id.statsLink)?.setOnClickListener {
            val intent = android.content.Intent(this, TestAttemptStatsActivity::class.java)
            startActivity(intent)
            overridePendingTransition(0, 0)
        }
    }
    ...
    override fun onResume() {
        ...
        // Tutorial overlay for Test page
        TutorialManager.showIfNeeded(
            this,
            key = "tutorial_test",
            message = "Když si budeš dostatečně jistý, tady jsou cvičné testy, zkus vždy získat maximum bodů!"
        )
    }
