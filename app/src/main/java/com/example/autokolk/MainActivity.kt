...
class MainActivity : AppCompatActivity() {
    ...
    private fun handleVideoDisplay(videoPath: String?) {
        try {
            ...
            currentVideoFile = File.createTempFile("video", ".mp4", cacheDir).apply {
                deleteOnExit()
            }
            ...
            Log.d("MainActivity", "Video file size: ${currentVideoFile!!.length()} bytes")
            
            // Stop any current playback
            questionVideo.stopPlayback()
            
            // Make container visible before setting video path
            videoContainer.visibility = View.VISIBLE
            
            // Set up video path and start playback
